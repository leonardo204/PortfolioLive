'use client'

import Link from 'next/link'
import { useLocale } from 'next-intl'

interface ProjectRefData {
  slug: string
  title: string
  description?: string
  techs?: string[]
  thumbnailUrl?: string
}

interface ProjectRefCardProps {
  data: unknown
}

function SingleCard({ project, locale }: { project: ProjectRefData; locale: string }) {
  return (
    <Link
      href={`/${locale}/portfolio/${project.slug}`}
      className="block px-4 py-3 hover:bg-[#f8f9fb] transition-colors"
    >
      <div className="flex items-start justify-between gap-2 mb-1">
        <span className="text-sm font-semibold text-[#2b3438] leading-snug">
          {project.title}
        </span>
        <span className="text-xs text-[#0053db] font-medium flex-shrink-0">
          {locale === 'en' ? 'View' : '보기'} →
        </span>
      </div>
      {project.description && (
        <p className="text-xs text-[#586065] leading-relaxed mb-2 line-clamp-2">
          {project.description}
        </p>
      )}
      {project.techs && project.techs.length > 0 && (
        <div className="flex flex-wrap gap-1">
          {project.techs.slice(0, 5).map((tech) => (
            <span
              key={tech}
              className="px-2 py-0.5 text-xs rounded-full bg-[#dbe1ff] text-[#0048bf] font-medium"
            >
              {tech}
            </span>
          ))}
          {project.techs.length > 5 && (
            <span className="px-2 py-0.5 text-xs rounded-full bg-[#f1f4f7] text-[#586065]">
              +{project.techs.length - 5}
            </span>
          )}
        </div>
      )}
    </Link>
  )
}

export function ProjectRefCard({ data }: ProjectRefCardProps) {
  const locale = useLocale()

  const projects: ProjectRefData[] = Array.isArray(data)
    ? (data as ProjectRefData[])
    : data && typeof data === 'object'
      ? [data as ProjectRefData]
      : []

  const valid = projects.filter(p => p.slug)
  if (valid.length === 0) return null

  return (
    <div className="my-3 rounded-xl border border-[#eaeef2] bg-white overflow-hidden shadow-sm">
      <div className="px-4 py-2.5 bg-[#f8f9fb] border-b border-[#eaeef2]">
        <span className="text-xs font-semibold text-[#586065] uppercase tracking-wide">
          {locale === 'en' ? 'Related Projects' : '관련 프로젝트'}
        </span>
      </div>
      <div className="divide-y divide-[#eaeef2]">
        {valid.map((project, i) => (
          <SingleCard key={project.slug ?? i} project={project} locale={locale} />
        ))}
      </div>
    </div>
  )
}
