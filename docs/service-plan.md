...
**인증**: 단일 관리자 (환경변수 기반 또는 NextAuth.js)
...
-- 경력 (Admin CRUD)
careers (id, company, company_type, department, position, location, started_at, ended_at, is_current, sort_order)
...
portfolio_projects (id, slug, title, description, category, technologies[], year, github_url, updated_at)

-- ═══ RAG ═══

-- 임베딩 (Portfolio + 회사 프로젝트 + 경력 데이터)
embeddings (id, source_type, source_id, section, content, embedding vector(768), metadata jsonb)
-- source_type: 'portfolio' | 'work_project' | 'career'

-- ═══ 채팅 ═══

chat_sessions (id, visitor_id, started_at, ended_at, message_count)
chat_messages (id, session_id, role, content, model_used, latency_ms, created_at)

-- ═══ 분석/보안 ═══

page_views (id, path, referrer, visitor_id, created_at)
chat_analytics (id, session_id, question_intent, satisfaction, created_at)
guardrail_events (id, session_id, violation_type, message, created_at)
...
services:
  ...
  agent:                  # FastAPI + LangGraph + CopilotKit Runtime
    ports: ["3101:8000"]
    depends_on: [postgres]
...
- [ ] PostgreSQL + pgvector 세팅 (private port 5433)
